...
def get_investor_data(symbol, date_str):
    """
    Fetch daily net purchases (媛쒖씤, ?멸뎅?? 湲곌?) from Naver Finance.
    """
    try:
        ...
        if len(cols) >= 7:
            ...
            # 0: Date, 1: 醫낃?, 2: ?꾩씪鍮? 3: ?깅씫瑜? 4: 嫄곕옒??            # 5: 湲곌? ?쒕ℓ留ㅻ웾, 6: ?몄씤 ?쒕ℓ留ㅻ웾, 7: ?멸뎅??蹂댁쑀二쇱닔 (sometimes column structure changes slightly)
            # It's safer to just pick them by their known indices:
            
            # On the 'frgn' tab:
            # 5th index = 湲곌? (Institution)
            # 6th index = ?멸뎅??(Foreigner)
            inst = cols[5].text.strip()
            foreign = cols[6].text.strip()
            
            # Since Naver doesn't explicitly show '媛쒖씤(Retail)' on this specific summary table easily, 
            ...
            for row in rows:
                ...
                if len(cols) >= 7:
                    ...
                    inst = cols[5].text.strip()
                    foreign = cols[6].text.strip()
                    recent_data.append(f"[{date_val}] 湲곌?: {inst}二? ?멸뎅?? {foreign}二?)
                    count += 1
            
            return {
                "理쒓렐_?숉뼢": " | ".join(recent_data),
                "is_realtime": False
            }
...
def is_relevant_article(title, stock_name):
    ...
    market_terms = ["肄붿뒪??, "肄붿뒪??, "吏??, "?쒗솴", "留덇컧", "?댁슃利앹떆", "釉붾（移?, "湲濡쒕쾶 利앹떆", "?꾩떆??利앹떆"]
    market_count = sum(1 for term in market_terms if term in title)
    
    # 2. Strict Subject Check (Primary focus on this stock)
    subject_patterns = [
        rf"\[.*{re.escape(stock_name)}.*\]", # [?쇱꽦?꾩옄]
        rf"{re.escape(stock_name)}\s*[:]",      # ?쇱꽦?꾩옄 :
        rf"^{re.escape(stock_name)}"          # ?쇱꽦?꾩옄 (start of title)
    ]
...
def scrape_naver_news(symbol, name, target_date_str, max_articles=20):
                ...
                for row in rows:
                    ...
                    if article_date_only == date_clean:
                        ...
                        if len(name) <= 2:
                            ...
                            # Avoid matching subsidiaries like "SK??, "SK?섏씠?됱뒪"
                            pattern = rf"(?:^|[^媛-?즑-zA-Z0-9]){re.escape(name)}(?:$|[^媛-?즑-zA-Z0-9])"
    ...
    if not articles:
        ...
        articles = [{"title": f"{name}, ?쒖옣 ?먮쫫 諛?愿???뚮쭏 遺꾩꽍", "url": f"https://finance.naver.com/item/news.naver?code={symbol}", "date": target_date_str.replace("-", ".") + " 09:00", "source": "利앷텒?뺣낫"}]
...
def select_impactful_article(stock_name, articles, change_val):
    ...
    """
    direction = "?곸듅" if change_val >= 0 else "?섎씫"
    ...
    if api_key and api_key != "your_api_key_here" and GENAI_AVAILABLE:
        try:
            ...
            prompt = (
                f"{stock_name}??二쇨?媛 ?ㅻ뒛 {direction}?덉뒿?덈떎. ?ㅼ쓬 ?댁뒪 ?ㅻ뱶?쇱씤 以?"
                f"??蹂?숈뿉 媛?????먯씤???섏뿀??寃껋쑝濡??먮떒?섎뒗 湲곗궗??踰덊샇(0遺???쒖옉)留??섎굹 怨⑤씪二쇱꽭??\n"
                f"**?듭떖 吏移?**\n"
                f"1. **?뚯궗 ?뱀젙???댁뒪 ?곗꽑**: '?ㅼ쟻', '?섏＜', '?몄닔/?⑸퀝', '?좎젣??, '?좉퀬媛 寃쎌떊' ??{stock_name} ?뚯궗 ?먯껜???뚯떇??理쒖슦?좎쑝濡??좏깮?섏꽭??\n"
                f"2. **?쒖옣/吏???꾩껜 ?댁뒪 諛곗젣**: '肄붿뒪???곸듅', '?쒗솴 留덇컧', '吏??5000 ?뚰뙆' ???쒖옣 ?꾩껜 ?먮쫫???ㅻ（???댁뒪??{stock_name} ?꾩슜 ?댁뒪媛 ?덈떎硫?臾댁“嫄??쒖쇅?섏꽭??\n"
                f"3. 留뚯빟 ?뚯궗? 愿?⑤맂 ?댁뒪媛 ?섎굹???녿떎硫?'none'?대씪怨??듯빐二쇱꽭??\n\n"
                + "\n".join([f"{i}: {a['title']}" for i, a in enumerate(articles)]) +
                "\n\n?묐떟? 諛섎뱶???レ옄 ?섎굹 ?먮뒗 'none'留??댁＜?몄슂."
            )
    ...
    company_keywords = ["?좉퀬媛", "理쒕떎二쇱＜", "?ㅼ쟻", "?섏＜", "?곸뾽?댁씡", "?묒옄", "諛곕떦", "?몄닔", "?⑸퀝", "怨듭떆", "?뱀쭠二?]
    market_keywords = ["肄붿뒪??, "吏??, "?쒗솴", "留덇컧", "?댁슃利앹떆"]
...
def generate_summary(stock_name, articles, change_val, best_idx=0, investor_data=None, analyst_data=None, market="KR"):
    ...
    if api_key and api_key != "your_api_key_here" and GENAI_AVAILABLE:
        try:
            ...
            if market == "US":
                prompt = (
                    f"?꾨Ц 湲덉쑖 遺꾩꽍媛濡쒖꽌 誘멸뎅 二쇱떇 {stock_name}??二쇨? {direction} ?듭떖 ?먯씤??遺꾩꽍?섏꽭??\n"
                    f"**?꾨옒 ?곸뼱 ?댁뒪 湲곗궗?ㅼ쓣 諛뷀깢?쇰줈 ?묒꽦?섏꽭??(泥?踰덉㎏ 湲곗궗媛 媛??以묒슂??:**\n\n"
                )
            else:
                prompt = (
                f"?뱀떊? 湲덉쑖 ?쒖옣??遺꾩꽍?섎뒗 ?꾨Ц AI 由ы룷?곗엯?덈떎. ?ㅼ쓬 二쇱떇 {stock_name} ({direction})??愿??理쒖떊 湲곗궗?ㅼ쓣 ?쎄퀬 遺꾩꽍 由ы룷?몃? ?묒꽦?섏꽭??\n\n"
                f"**遺꾩꽍???댁뒪 ?곗씠??**\n"
            )
            ...
            for i, article in enumerate(reordered[:limit]):
                ...
                prompt += f"湲곗궗 {i+1}: {title}\n?댁슜: {content[:1500]}\n\n"
                
            if market == "KR" and investor_data:
                time_ctx = "?μ쨷 ?ㅼ떆媛??щ·留??쒖젏)" if investor_data.get('is_realtime') else "??留덇컧(醫낃?)"
                prompt += f"**?ㅻ뒛 ?섍툒 ?곗씠??({time_ctx} 湲곗? ?쒕ℓ???쒕ℓ??:**\n"
                prompt += f"- 媛쒖씤: {investor_data['媛쒖씤']}\n"
                prompt += f"- ?멸뎅?? {investor_data['?멸뎅??]}\n"
                prompt += f"- 湲곌?: {investor_data['湲곌?']}\n\n"
                
            prompt += (
                f"**?묒꽦 媛?대뱶?쇱씤 (諛섎뱶??以??:**\n"
                f"1. **?쒓뎅??踰덉뿭 諛?醫낇빀 ?붿빟 (summary)**: 紐⑤뱺 ?곷Ц ?댁뒪 ?댁슜? 諛섎뱶??癒쇱? ?먯뿰?ㅻ윭???쒓뎅?대줈 踰덉뿭?섏꽭?? ?댄썑 湲곗궗?ㅼ쓽 ?듭떖 ?댁슜??2~3臾몄옣 臾몄껜濡?醫낇빀?섏뿬 ?붿빟?섏꽭?? ?⑥닚 踰덉뿭泥닿? ?꾨땶, ?쒓뎅 ?낆옄媛 ?쎄린 ?명븳 ?꾨Ц媛???댁“瑜??ъ슜?섏떗?쒖삤.\n"
                f"2. **?ㅼ썙???붿빟 (short_reason)**: ?꾩뿉???묒꽦??'?붿빟(summary)'???듭떖 ?댁슜??**2~3媛쒖쓽 紐낆궗???댁젅/?⑥뼱**濡쒕쭔 ?뺤텞?섏꽭?? (?? '留ㅼ텧 ?깆옣??吏?? ?ㅼ쟻 湲곕?媛?, '?좎젣??異쒖떆 ?④낵, ?먯쑀???뺣?', '湲濡쒕쾶 寃쎄린 移⑥껜, 留ㅼ텧 ?뷀솕 ?곕젮'). 臾몄옣?대굹 留덉묠?쒕? ?ъ슜?섏? 留먭퀬 紐낆궗?뺤쑝濡????딆뼱???묒꽦?섏꽭??\n"
                f"3. **移댄뀒怨좊━ 遺꾨쪟 (category)**: ?쒓났???댁뒪 ?깃꺽???곕씪 '?ㅼ쟻', '?섍툒', '?댁뒋', '嫄곗떆寃쎌젣', '鍮낇뀒?? 以?媛???곸젅???섎굹瑜??좏깮?섏꽭??"
            )
            
            if market == "KR" and investor_data:
                prompt += " ?쒓났???멸뎅??湲곌????섍툒 ?숉뼢??二쇨???誘몄튇 ?곹뼢(留ㅼ닔??留ㅻ룄??以묒떖)???붿빟臾??띿뿉 ?먯뿰?ㅻ읇寃??뱀뿬?댁꽭??"
                
            prompt += (
                f"\n4. **湲덉? ?ы빆**: '二쇨?媛 ?щ옄?듬땲?? 媛숈? 六뷀븳 寃곌낵 ?섏뿴? ?쇳븯怨? '?? 蹂?숉뻽?붿? ?댁뒪??湲곕컲??援ъ껜??洹쇨굅瑜??쒖떆?섏꽭??\n"
                f"5. **異쒕젰 ?뺤떇**: ?꾨옒 JSON 援ъ“濡쒕쭔 ?듬??섏꽭?? 留덊겕?ㅼ슫 湲고샇(```json)???ㅻⅨ ?띿뒪?몃뒗 ?쇱껜 ?ы븿?섏? 留덉꽭??\n"
                f"{{\"category\": \"移댄뀒怨좊━\", \"short_reason\": \"?듭떖 ?ㅼ썙???댁젅 (2~3媛?\", \"summary\": \"洹쒖튃??以?섑븳 ?먯뿰?ㅻ윭???쒓? ?붿빟\"}}"
            )
            ...
            if response and response.text:
                ...
                try:
                    ...
                    return {"category": "?댁뒋", "short_reason": f"?쒖옣 ?섍툒 諛??낇솴 蹂?붿뿉 ?곕Ⅸ {direction}", "summary": response.text.strip()} # fallback
        except TimeoutError:
    ...
    main_news = news_titles[best_idx] if has_valid_news else "?쒖옣 ?섍툒 蹂??
    
    if change_val >= 0:
        templates = [
            f"{stock_name}?(?? {main_news} ?뚯떇???꾪빐吏硫?留ㅼ닔?멸? 媛뺥솕?섏뿀?듬땲??",
            f"?ㅻ뒛 {stock_name} 二쇨???{main_news} ?깆씠 ?몄옱濡??묒슜?섎ŉ 湲띿젙?곸씤 ?먮쫫??蹂댁??듬땲??"
        ]
    else:
        templates = [
            f"{stock_name}?(?? {main_news} ?ы뙆濡??명빐 留ㅻ룄 ?뺣젰???믪븘吏硫??쎌꽭瑜?蹂댁??듬땲??",
            f"{stock_name} 二쇨???{main_news} ?깆뿉 ?곕Ⅸ ?ъ옄 ?щ━ ?꾩텞?쇰줈 ?섎씫 留덇컧?덉뒿?덈떎."
        ]
    ...
    keyword_fallback = "?섍툒 蹂?? ?낇솴 蹂??
    if has_valid_news:
        words = [w for w in main_news.split() if len(w) > 1]
        if len(words) >= 2:
            keyword_fallback = f"{words[0]}, {words[1]}"
            
    return {"category": "?댁뒋", "short_reason": keyword_fallback, "summary": fallback_text}
...
def generate_short_reason(stock_name, articles, change_val, best_idx=0, translated_title=None):
    if translated_title:
        ...
        return f"{words[0]}, {words[1]}" if len(words) >= 2 else "?낇솴 蹂?? ?섍툒 蹂??
...
def get_related_stocks(symbol, name, date_str, theme=None, market="KR"):
    ...
    # - If name starts with same 2 chars (KR) -> [洹몃９??
    # - Otherwise -> [寃쎌웳??
...
def generate_daily_json(date_str=None, market="KR"):
    ...
    for idx, stock in enumerate(movers):
        ...
        signal_cat = summary_obj.get("category", "?댁뒋")
        short_reason = summary_obj.get("short_reason", f"?낇솴 蹂?붿뿉 ?곕Ⅸ ?곸듅" if change_val >= 0 else f"?낇솴 蹂?붿뿉 ?곕Ⅸ ?섎씫")
